...
def test_classify_geometry():
    print(flush=True)
    assert CBRdb.classify_geometry(molecule("H")) == 'monatomic'
    assert CBRdb.classify_geometry(molecule("N2")) == 'linear'
...
def test_mace_free_energy():
    ...
    atoms = molecule('H2O')
    g_h2o, _, _, _ = CBRdb.free_energy_mace(atoms)
    print(f"Gibbs free energy: {g_h2o}", flush=True)

    atoms = molecule('H2')
    g_h2, _, _, _ = CBRdb.free_energy_mace(atoms)
    print(f"Gibbs free energy: {g_h2}", flush=True)

    atoms = molecule('O2')
    g_o2, _, _, _ = CBRdb.free_energy_mace(atoms)
