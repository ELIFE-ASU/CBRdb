...
def test_spin_multiplicity():
    print(flush=True)
    examples = {
        "[C]": "isolated carbon atom (triplet)",
        "C": "methane (singlet)",
        "[CH3]": "methyl radical (doublet)",
...
def test_sym_number():
    ...
    s = CBRdb.get_symmetry_number(molecule("H2O"))
    print(s)  # -> 2  (C2v)
    assert s == 2

    s = CBRdb.get_symmetry_number(molecule("C2H4"))
    print(s)  # -> 4  (D2h)
    assert s == 4

    s = CBRdb.get_symmetry_number(molecule("CH4"))
    print(s)  # -> 12 (Td)
    assert s == 12

    s = CBRdb.get_symmetry_number(molecule("N2"))
    print(s)  # -> 2  (D∞h)
    assert s == 2

    s = CBRdb.get_symmetry_number(molecule("CO"))
    print(s)  # -> 1  (C∞v)
    assert s == 1

    s = CBRdb.get_symmetry_number(molecule("C6H6"))
    print(s)  # -> 12 (D6h)
    assert s == 12
...
def test_classify_geometry():
    print(flush=True)
    assert classify_geometry(molecule("H")) == 'monatomic'
    assert classify_geometry(molecule("N2")) == 'linear'
    assert classify_geometry(molecule("CO2")) == 'linear'
    assert classify_geometry(molecule("H2O")) == 'nonlinear'
    assert classify_geometry(molecule("HCN")) == 'linear'
